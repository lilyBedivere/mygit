...
namespace CSharpModBusExample
{
    class DB
    {
        ...
        {
            string cmdstr = "Select " + cell + " from " + table + " where UserN='" + user + "'";
        ...
        {
            string cmdstr = "Select " + cell + " from " + table + " where UserName='" + user + "'";
            // "Select "+cell+" from " + table + "where UserN='"+user+"'";
            SqlCommand myCmd = new SqlCommand(cmdstr, GetCon());
            string strdata = ExecScalar(myCmd);
            return strdata;
        ...
        public void Databaseoperation(string sqlStr)
        {
            SqlConnection myConn = GetConnection();
            try
            {
                myConn.Open();
                var myCommand = new SqlCommand(sqlStr, myConn);//声明SqlCommand对象实例
        ...
        public void Cmdsorted()
        {
            const string smsSqlstr删除 = "delete from  order_new"; //清空当当前新建订单表
            Databaseoperation(smsSqlstr删除);
